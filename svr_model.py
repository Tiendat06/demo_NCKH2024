...
yolov6_model = my_yolov6("./weights/best_ckpt.pt","cpu","./data/vinbigdata.yaml", 512, True)
...
def home_page():
    ...
    if request.method == "POST":
         try:
            ...
            if image:
                # Lưu file
                path_to_save = os.path.join("static\img", image.filename)
                ...
                frame, ndet, label_name, percentage, sick_name = yolov6_model.infer(frame, conf_thres=0.3, iou_thres=0.4)
                ...
                if ndet!=0:
                    ...
                    return render_template("index.html", user_image = image.filename , rand = str(random()),
                                           msg="Tải file lên thành công", ndet = ndet, label=label_name)
                else:
